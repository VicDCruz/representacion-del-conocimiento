...
reset_KB_supermarket_ricardo:-
	open_supermarket_kb_ricardo(KB),
	save_kb(KB).
...
initialize_KB_supermarket_ricardo:-
	reset_KB_supermarket_ricardo,
	assign_func_value(empty).
...
convert_cartesian_to_cylindrical(X,Y,Z,R,O) :-
	...
	O is round(Otemp*100)/100,    %Round o with two decimal positions	
	assign_func_value([]).
convert_cartesian_to_cylindrical_yolo(X,Y,Z,R,O) :-
	Rtemp is sqrt(((100*X)**2)+((100*Y)**2)), %Calculate r
	R is round(Rtemp*100)/100,                %Round r with two decimal positions
	Otemp is atan(Y/X)*57.29,     %Calculate o	
	O is round((Otemp-90)*100)/100,           %Round o with two decimal positions	
	assign_func_value(empty).
...
%%%%%%%%%%%%%%%%%% DEMO Action Reasoner %%%%%%%%%%%%%%%%%%%%%%%%
:- consult('$SITLOG_HOME/apps/german_open_2019/demo_supermarket/action_reasoner.pl').
...
get_amcl :-
    ...
    | otherwise-> 
      ...
      rosswipl_to_ioca(JsonRes,[X,Y,Theta]),
      X_f is round(X*100)/100,
      Y_f is round(Y*100)/100,
      Theta_f is round(Theta*100)/100,
      PositionAMCL = [X_f,Y_f,Theta_f]
    ),
...
get_person_odometry_yolo([R,O,P],D) :-
    ( mode_exe(test) -> 
      Rf is round(R-D),
      Of is round(O),
      Position = [Rf,Of,P]
    | otherwise-> 
      ...
      Rf is round(R-D),
      Of is round(O),
      ioca_to_rosswipl(person_pose(Rf,Of,P),ReqJsonString),
      print(ReqJsonString),nl,
      rosswipl_call_service('/get_person_pose_amcl',ReqJsonString,JsonRes),
      rosswipl_to_ioca(JsonRes,Position)
    ),
    assign_func_value(Position).
...
add_location_kb(_,_) :- assign_func_value(empty).
...
check_not_empty_hypothesys(Hyp,Conf):-
	print('Hyp:'),print(Hyp),nl,print('Conf:'),print(Conf),nl,
...
inference_demo_parser('i will put a coke a malz noodles and bisquits'):-
    assign_func_value(update_inventory([coke,malz,noodles,bisquits])).
...
inference_demo_parser('bring me a malz'):-
    assign_func_value(bring_me_without_verification([malz])).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Supermarket demo functions Ricardo
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
rsupermarket_parser('bring me a coke and a bag of crisps'):-
    assign_func_value([find(object(coke)),take(object(coke)),find(object(nodless)),take(object(noodles)),deliver(destiny(human(H),location(front_desk)))]).

%1
rsupermarket_parser('place the coke in the shelve of drinks'):-
    assign_func_value([find(object(coke)),take(object(coke)),deliver(object(coke),destiny(location(shelf_drinks)))]).
...
%2
preferences_demo_parser('bring me malz and something to eat'):-
    assign_func_value([malz,food]).

%3
preferences_demo_parser('bring me noodles and something to drink'):-
    assign_func_value([noodles,drink]).

%4
preferences_demo_parser('bring me bisquits and something to drink'):-
    assign_func_value([bisquits,drink]).

%5
preferences_demo_parser('bring me something to eat and a coke'):-
    assign_func_value([food,coke]).

%6
preferences_demo_parser('bring me something to eat and malz'):-
    assign_func_value([food,malz]).

%7
preferences_demo_parser('bring me something to drink and noodles'):-
    assign_func_value([drink,noodles]).

%8
preferences_demo_parser('bring me something to drink and bisquits'):-
    assign_func_value([drink,bisquits]).

%9
preferences_demo_parser('bring me a coke and noodles'):-
    assign_func_value([coke,noodles]).

%10
preferences_demo_parser('bring me malz and noodles'):-
    assign_func_value([malz,noodles]).

%11
preferences_demo_parser('bring me a coke and bisquits'):-
    assign_func_value([coke,bisquits]).

%12
preferences_demo_parser('bring me malz and bisquits'):-
    assign_func_value([malz,bisquits]).

%13
preferences_demo_parser('bring me noodles and a coke'):-
    assign_func_value([noodles,coke]).

%14
preferences_demo_parser('bring me noodles and malz'):-
    assign_func_value([noodles,malz]).

%15
preferences_demo_parser('bring me a bisquits and coke'):-
    assign_func_value([bisquits,coke]).

%16
preferences_demo_parser('bring me a bisquits and malz'):-
    assign_func_value([bisquits,malz]).

%17
preferences_demo_parser('bring me something to drink and something to eat'):-
    assign_func_value([drink,food]).

%18
preferences_demo_parser('bring me something to eat and something to drink'):-
    assign_func_value([food,drink]).


%% ==> PREFERENCE Demo
%% Build the dialogue within the demo
dialogue_conceptual_inference(Answer):-
    ( Answer=='hi golem i like malz'->Return='excellent, malz is much healthier than the coke'
    ; Answer=='i like noodles best'->Return='great, I will recall your choice'
    ; Return='I do not undestand'
    ),
...
prompt_user_to_decide(Element_Asked,Pref_elem):-
    ((Element_Asked=='drink';Element_Asked=='food') ->
       Ans = ['ok, I will bring you',Pref_elem,', your favorite',Element_Asked]
     | otherwise ->
       Ans = ['but you like',Pref_elem,'better than',Element_Asked,', shall I bring you the',Element_Asked,'?']
    ),
    assign_func_value(Ans).
...
head_of_list([]):-
    nl,nl,print('empty list'),nl,nl,
    assign_func_value(empty_list).
    
head_of_list([H|_]):-
    nl,nl,print('Head: '),print(H),nl,nl,
    assign_func_value(H).
...
%% ==> PREFERENCE Demo
%% Saves a new preference in the KB
save_new_pref(Ind,Pri):-
    open_kb(KB),
    class_of_an_object(Ind,KB,Class),    
    change_weight_class_property_preference(Class,'-'=>>to_serve=>Ind,Pri,KB,KB_1),
    save_kb(KB_1),
    assign_func_value(empty).


%% ==> PREFERENCE Demo
%% Gets the list of individuals in Class removing those in List
get_objs_then_substract_order(Class,List):-
    open_kb(KB),
    objects_of_a_class(Class,KB,Individuals),
    get_objs_then_substract_order_aux(List,Individuals,Final),
    assign_func_value(Final).

get_objs_then_substract_order_aux([],L,L):-!.

get_objs_then_substract_order_aux([H|T],L,R):-
    remove_element(H,L,Lst),
    get_objs_then_substract_order_aux(T,Lst,R).


%% ==> PREFERENCE Demo
%% Checks if Obj is in its default place. If so, the task continues. Otherwise, the user is  asked if he wants the robot to reposition Obj
is_last_seen_correct(_,More,unknown,_) :-
    assign_func_value(check(More)).

is_last_seen_correct(Obj,_,Last,List) :-
    List = [_,Loc2|_],
    Last \== Loc2,
    assign_func_value(add_misplaced(Obj,Last,Loc2)).

is_last_seen_correct(_,More,_,_) :-
    assign_func_value(check(More)).


%% ==> PREFERENCE Demo
%% Add 'misplaced' property to Obj 
add_misplaced_property(Obj):-
    open_kb(KB),
    add_object_property(Obj,misplaced,yes,KB,NewKB),
    save_kb(NewKB),
    assign_func_value(empty).


%% ==> PREFERENCE Demo
%% Remove 'misplaced' property from Obj 
rm_misplaced_property(Obj):-
    open_kb(KB),
    rm_object_property(Obj,misplaced,KB,NewKB),
    save_kb(NewKB),
    assign_func_value(empty).


%% Llamada a change_kb_extension
%% ==> PREFERENCE Demo
%% Abductive reasoning
consult_kb_abductive_object_property(Object,Property,Explanation):-
    open_kb(KB),
    object_abductive(Object,KB,List),
    aux_abductive_object_property(Property,List,Explanation),
    assign_func_value(empty).

aux_abductive_object_property(_,[],'no explanation'):-!.

aux_abductive_object_property(Propty,[Propty:(Explanation)|_],Explanation):-!.

aux_abductive_object_property(Propty,[_|Rest],Explanation):-
    aux_abductive_object_property(Propty,Rest,Explanation).
...
%%%
%%% Author: Noe Hdez
min_robot_height(1.10). %Minimun height
...
aux_func([object(Name,X,Y,Z,_,_,_,_,_)|More_New], Old_Cat, Scan, Tilt, RobotHeight, R1):-
    ...
    min_robot_height(MinH),
    max_robot_height(MaxH),
    cam2arm(right, RobotHeight, [X,Y,Z], [Tilt,0.11,0.10], [-0.05,0.22,0.2248], [Height, _, _]),
    MinH =< Height, Height =< MaxH,     
...
%% Author: Noe Hernandez
%% Given the list of tuples where categories are part of,
%% return the list of single categories
extract_all_cat_st(Tuples):-
    aux_extract_all_cat_st(Tuples,ListCat),
    assign_func_value(ListCat).

aux_extract_all_cat_st([],[]).

aux_extract_all_cat_st([categval(Category,_,_)|Rest],[Category|RestCat]):-
    aux_extract_all_cat_st(Rest,RestCat),!.
    
%% Author: Noe Hernandez
%% If Val is in List, this function returns the list that results of
%% deleting the values of List up to Val (deletes Val)
list_fil(List, Val):-
...
%% Author: Noe Hernandez
%% If Val is in List, this function returns the list that results of
%% deleting the values of List up to Val (keeps Val)
list_hor(List, Val):-
...
check_flag_value(0.0, Angle):-
    ( Angle == 0.0 ->
         var_op(set(turn_flag_approach, 1.0))
     | otherwise -> 
         var_op(set(turn_flag_approach, -1.0))
    )
.
...
kind_order(Sentence) :-
    ...
    ( member(kellogs,List) ->
     ...
     | otherwise ->
       assign_func_value( get_order([noodles, malz]) )
    )
.
...
historia(_):-get_history(History),
	     assign_func_value(History).
	     
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


%% Clean up user function 1
clean_up_f1(Hght,Obj,Rest_Obj,Remaining):-
    min_robot_height(MinH),
    max_robot_height(MaxH),
    MinH=<Hght,Hght=<MaxH,
    assign_func_value(check_loc(Obj,Rest_Obj,Remaining)).


clean_up_f1(_,_,Rest_Obj,Remaining):-
        assign_func_value(check_height(Rest_Obj,Remaining)).

clean_up_f2(Pos,Rem_Pos):-
    aux2_clean_up(Pos,Rem_Pos,Val),
    assign_func_value(Val).

aux2_clean_up([],_,[]).

aux2_clean_up([H|T],T,[H|T]).

aux2_clean_up([_|T],L,V):-
    aux2_clean_up(T,L,V).


%% Take out the garbage
next_location_tog(bin1):-
    R = move_to_bag(bin2,1),
    assign_func_value(R).
next_location_tog(bin2):-
    R = move_to_collect,
    assign_func_value(R).

%% Where is this
whereis_get_fine_directions(Object):-
    open_kb(KB),
    object_property_value(Object,fine_directions,KB,FineDir),
    assign_func_value(FineDir).

whereis_get_directions(Object):-
    open_kb(KB),
    object_property_value(Object,directions,KB,Dir),
    assign_func_value(Dir).

whereis_get_directions_from(Object,From):-
    open_kb(KB),
    atom_string(From,SFrom),
    string_concat('directions_',SFrom,FinalString),
    atom_string(FinalAtom,FinalString),
    print("Final String------------->"),
    print(FinalString),
    object_property_value(Object,FinalAtom,KB,Dir),
    assign_func_value(Dir).

whereis_get_room(Object):-
    open_kb(KB),
    class_of_an_object(Object,KB,Room),
    assign_func_value(Room).
...
wii_parser(X,H):-
   from_atom_to_list(X,List),
   reverse(List,[H|NewList]),
   %Y='i hate you',
   assign_func_value(H).
...
fetch_time_over :- 	
	...
	check_time_over(MaxTime, [Hour, Min, Sec]),
	assign_func_value(true).

fetch_time_over :-	
	assign_func_value(false).

check_time_over([MaxHr,MaxMin,MaxSec],[MaxHr,MaxMin,MaxSec]).	

%Recepcionist

recepcionist_name_new_guest(Name):-
    open_kb(KB),
    change_value_object_property(the_party,new_guest,Name,KB,NewKB),
    save_kb(NewKB),
    assign_func_value(empty).
    
recepcionist_drink_new_guest(Drink):-
    open_kb(KB),
    object_property_value(the_party,new_guest,KB,Name),
    change_value_object_property(Name,favorite_drink,Drink,KB,NewKB),
    save_kb(NewKB),
    assign_func_value(empty).

recepcionist_get_name_new_guest:-
    open_kb(KB),
    object_property_value(the_party,new_guest,KB,Name),
    object_property_value(Name,mcs_id,KB,Id),
    assign_func_value([Name,Id]).
    
recepcionist_introduce_guest(Name):-
    open_kb(KB),
    object_property_value(Name,favorite_drink,KB,Drink),
    assign_func_value( say(['He is',Name,'and his favorite drink is',Drink]) ).
    
recepcionist_reset_chairs:-
    open_kb(KB),
    object_property_value(the_party,sit_places,KB,Chairs),
    change_value_object_property(the_party,free_places,[],KB,NewKB),
    assign_func_value(Chairs).
    
recepcionist_save_id_new_guest(Name,X):-
    open_kb(KB),
    add_object_property(Name,mcs_id,X,KB,NewKB),
    save_kb(NewKB),
    assign_func_value(empty).

recepcionist_get_id_new_guest:-
    open_kb(KB),
    object_property_value(the_party,mcs_id,KB,Name),
    assign_func_value(Name).